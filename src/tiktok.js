import { fetchTikTok } from './api.js';
...
export function initTikTok() {
  ...
  function setMode(mode) {
    ...
    if (mode === 'single') {
      ...
      tikBatchInput.classList.remove('show');
    } else {
      tikUrlInput.style.display = 'none';
      tikBatchInput.style.display = 'block';
      tikBatchInput.classList.add('show');
    }
    clearResult();
    clearBatchStatus();
    hideError();
  ...
  function hideError() { tikError.classList.remove('show'); tikError.innerHTML = ''; }
  ...
  async function processBatch(links) {
    ...
    if (successCount > 0) {
      ...
      btnAll.addEventListener('click', function() {
        ...
        if (downloadLinks.length === 0) { showToast('Không có video nào để tải!', 'info'); return; }

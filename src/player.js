...
export class MusicPlayer {
    ...
    if (this.audio) {
      this.audio.addEventListener('loadedmetadata', () => {
        ...
        if (this.audio.src && this.currentIndex >= 0) {
          ...
          this.audio.play().catch(e => console.warn('Autoplay bị chặn:', e));
          this.startProgress();
  ...
  setDefaultSong() {
    ...
    const t = getTranslation('player_no_song');
    if (this.songName) this.songName.textContent = t || 'Chưa có nhạc';
    if (this.artistName) this.artistName.textContent = getTranslation('player_user') || 'Người dùng';
  ...
  addToPlaylist(name, artist, src, img) {
    ...
    if (isDuplicate) {
      showToast(`Bài hát "${name}" - "${artist}" đã có trong danh sách.`, 'info', 3000);
      return false;
    }
    ...
    const load = () => {
      ...
      if (this.audio) {
        if (song.src) {
          ...
          this.audio.load();
          if (this.isPlaying) { this.audio.play().catch(e => console.warn('Play error:', e)); }
        } else {
          this.audio.src = '';
          this.audio.load();
          this.isPlaying = false;
  ...
  togglePlay() {
    if (this.currentIndex < 0 || this.playlist.length === 0) {
      showToast('Danh sách phát trống.', 'info', 3000);
      return;
    }
    ...
    if (this.isPlaying) {
      if (this.playIcon) this.playIcon.className = 'fas fa-pause';
      if (this.audio) { this.audio.play().catch(e => console.warn('Play error:', e)); }
      this.startProgress();
    } else {
      if (this.playIcon) this.playIcon.className = 'fas fa-play';
  ...
  updateDisplay() {
    ...
    const total = formatTime(this.totalTime);
  ...
  prev() {
    ...
    if (this.isPlaying && this.audio) { this.audio.play().catch(e => console.warn('Play error:', e)); }
  }
  ...
  removeFromPlaylist(index) {
    ...
    if (this.currentIndex === index) {
      if (this.playlist.length > 0) {
        ...
        this.isPlaying = false;
        if (this.playIcon) this.playIcon.className = 'fas fa-play';
        const t = getTranslation('player_no_song') || 'Chưa có nhạc';
        if (this.songName) this.songName.textContent = t;
        if (this.artistName) this.artistName.textContent = getTranslation('player_user') || 'Người dùng';
        if (this.cover) this.cover.innerHTML = '<i class="fas fa-music"></i>';
        this.updateDisplay();
      }
